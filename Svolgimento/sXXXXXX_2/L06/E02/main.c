...
mark menu(atletaS *sp, int nAtleti, menuE comando,mark Ord);
...
int main(void)
{
    ...
    do {
        ...
        if((sceltaM!=m_esci)&&(sceltaM<m_errMenu)&&(sceltaM>=m_print))
            Ordine=menu(sp,nAtleti,sceltaM,Ordine);
...
mark menu(atletaS *sp, int nAtleti, menuE comando,mark Ord)
{
    void *p=sp,*pcogn,*pdata,*pcodice;
    int chiave_i,indice,*indOmonimi;
    char *chiave_c;

    Ord = malloc(Ord_Not*sizeof(mark));
    ...
    switch(comando){
        ...
        case m_fprint:
            ...
            break;
        case m_ordData:
            pdata = (int *) creaVettP(&(sp->data),nAtleti,INTEROP);
            bubbleSortInt(pdata,0,nAtleti);
            prints(pdata,0,nAtleti,stdout);
            Ord[Ord_per_data]=ORDINATO;
            break;
        case m_ordCodice:
            pcodice = (int *) creaVettP(&(sp->codice),nAtleti,INTEROP);
            bubbleSortInt((int **)pcodice,0,nAtleti);
            prints(pcodice,0,nAtleti,stdout);
            Ord[Ord_per_Codice]=ORDINATO;
            break;
        case m_ordCognome:
            pcogn = (char *) creaVettP(&(sp->cogn),nAtleti,CARATTEREP);
            bubbleSortChar((char **)pcogn,0,nAtleti);
            if((indOmonimi=trovaOmonimi(pcogn,nAtleti))!=NULL){
                pcogn = (char *) creaVettP(&(sp->nome),nAtleti,CARATTEREP);
                ordinaOmonimi(pcogn,indOmonimi);
            }
            prints(pcogn,0,nAtleti,stdout);
            Ord[Ord_per_cognome]=ORDINATO;
            break;
        case m_leggiCat:
        ...
        case m_ricCodice:
            ...
            if(Ord[Ord_per_Codice]==ORDINATO){
        ...
        case m_ricCogn:
            ...
            if(Ord[Ord_per_cognome]==ORDINATO){
